...
from models.forward_tacotron_old import ForwardTacotronOld
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_2', type=str, default=None, help='[string/path] path to .pt model file.')
    ...
    checkpoint_2 = torch.load(checkpoint_path_2, map_location=torch.device('cpu'))
    config_2 = checkpoint_2['config']
    tts_model_2 = ForwardTacotronOld.from_config(config_2)
    tts_model_2.load_state_dict(checkpoint_2['model'])

    dsp = DSP.from_config(config_2)
    ...
    for i, x in enumerate(texts, 1):
        ...
        gen_pitch = tts_model.generate(x=x,
                                 semb=torch.from_numpy(semb).unsqueeze(0),
                                 alpha=args.alpha,
                                 pitch_function=pitch_function,
                                 energy_function=energy_function)

        pitch_function = lambda x: gen_pitch['pitch']
        energy_function = lambda x: gen_pitch['energy']
        gen = tts_model_2.generate(x=x,
                                   alpha=args.alpha,
                                   pitch_function=pitch_function)
