...
class ForwardTacotronOld(nn.Module):
    ...
    def generate(self,
        ...
        with torch.no_grad():
            ...
            pitch_hat = self.pitch_pred(x).transpose(1, 2)
            print(f'x {x.size()}')
            print(f'pitch hat {pitch_hat.size()}')
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> 'ForwardTacotronOld':
        model_config = config['forward_tacotron']['model']
        model_config['num_chars'] = len(phonemes)
        model_config['n_mels'] = config['dsp']['num_mels']
        return ForwardTacotronOld(**model_config)

    @classmethod
    def from_checkpoint(cls, path: Union[Path, str]) -> 'ForwardTacotronOld':
        checkpoint = torch.load(path, map_location=torch.device('cpu'))
        model = ForwardTacotronOld.from_config(checkpoint['config'])
