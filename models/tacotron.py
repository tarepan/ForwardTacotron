...
class CBHG(nn.Module):
    ...
    def forward(self, x):
        ...
        self._flatten_parameters()  # overerngineered?
        ...
        # self.rnn.flatten_paramenters() # instead of self.flatten...
        # And then the RNN
        x, _ = self.rnn(x)
        return x
...
class Tacotron(nn.Module):
    def __init__(self, embed_dims, num_chars, encoder_dims, decoder_dims, n_mels, fft_bins, postnet_dims,
        ...
        self.encoder = Encoder(embed_dims=embed_dims,
                               num_chars=num_chars,
    ...
    def forward(self, x, m, ref_mels):
        ...
        style_embed, style_attn_scores = self.gst(ref_mels)  # [N, 256]
    ...
    def generate_with_scores(self, x, scalars, steps=2000):
        ...
        # unnecessary matmuls in the decoder loop
        encoder_seq = self.encoder(x)
        style_embed, style_attn_scores = self.gst.forward_with_scores(scalars=scalars, batch_size=1)  # [N, 256]
        style_embed = style_embed.expand_as(encoder_seq)
        conditioned_encoder_seq = encoder_seq + style_embed
        encoder_seq_proj = self.encoder_proj(conditioned_encoder_seq)
    ...
    def generate(self, x, ref_mel, steps=2000):
        ...
        # unnecessary matmuls in the decoder loop
        encoder_seq = self.encoder(x)
        style_embed, style_attn_scores = self.gst(ref_mel)  # [N, 256]
        style_embed = style_embed.expand_as(encoder_seq)
        conditioned_encoder_seq = encoder_seq + style_embed
        encoder_seq_proj = self.encoder_proj(conditioned_encoder_seq)
        # encoder_seq_proj = self.encoder_proj(encoder_seq)
